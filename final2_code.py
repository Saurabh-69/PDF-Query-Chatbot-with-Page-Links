...
def retry_dialogflow_response(chat_wrapper, user_message, attempts=3):
    ...
    modified_response = bot_response+" \nFor further reference and support you can click on Open Pdf Button or copy paste the given link to open the relevant page. Thankyou!"
...
with gr.Blocks() as demo:
    ...
    def submit_query(query):
        ...
        if page_num is not None:
            ...
            return relevant_text, related_images, page_num, gr.update(visible=True), page_url  # Return the URL for the textbox
        else:
            return relevant_text, related_images, None, gr.update(visible=False), ""  # Set URL to empty if no page number
